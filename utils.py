...
# Label map
voc_labels = ("botol kaca", "botol plastik", "kaleng", "kertas")
...
def create_data_lists(dataset_path, output_folder):
    ...
    :param dataset_path: path to the 'VOC2007' folder
    ...
    # Training data
    for paths in os.listdir(dataset_path):
        ...
        path = os.path.join(dataset_path, paths)
        if paths == 'annotations':
          for file in os.listdir(path):
            objects = parse_annotation(os.path.join(path, file))
            if len(objects['boxes']) == 0:
                print('No objects found in the annotation file %s' % file)
                continue
